 ...
 */
function replaceQuotes(obj: { [key: string]: any }): { [key: string]: any } {
 ...
 */
function isValidJSONString(data: string): boolean {
 ...
 */
function isValidJSONObject(data: any): boolean {
 ...
 */
function isValidJSONArray(arr: (string | object)[]): boolean {
 ...
 */
function areAllValidJSON(arr: (string | object)[]): boolean {
  try {
    for (const element of arr) {
      if (typeof element === 'string') JSON.parse(element);
      else if (typeof element === 'object') {
        if (element instanceof Array && !isValidJSONArray(element)) return false;
        else if (!(element instanceof Object)) return false;
      } else return false;
    }
    return true;
  } catch {
 ...
 */
function convertJsonStringsToObject(jsonArray: Array<any>): Array<object> {
 ...
 */
export async function isJson(data: string | object): Promise<string | undefined> {
  try {
    ...
    if (data instanceof Uint8Array) throw new Error("Invalid JSON data.");
